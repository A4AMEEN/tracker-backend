...
router.get('/', async (req, res) => {
  try {
    ...
    const enriched = await Promise.all(users.map(async (u) => {
      ...
      // Use manually-entered inIndiaDays / inUAEDays from each record
      const daysInIndia = trips.reduce((s, t) => s + (t.inIndiaDays || 0), 0);
      const daysInUAE   = trips.reduce((s, t) => s + (t.inUAEDays   || 0), 0);
      return { ...u.toJSON(), tripCount: trips.length, daysInIndia, daysInUAE, totalDays: daysInIndia + daysInUAE };
    }));
...
router.post('/', async (req, res) => {
  try {
    ...
    const existing = await User.findOne({ firstName: { $regex: `^${firstName}$`, $options: 'i' }, lastName: { $regex: `^${lastName}$`, $options: 'i' } });
    if (existing) return res.status(409).json({ success: false, error: `User "${firstName} ${lastName}" already exists` });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (oldFullName !== newFullName) {
      await Trip.updateMany({ username: { $regex: `^${oldFullName}$`, $options: 'i' } }, { $set: { username: newFullName, designation: user.designation } });
    } else if (designation !== undefined) {
      await Trip.updateMany({ username: { $regex: `^${newFullName}$`, $options: 'i' } }, { $set: { designation: user.designation } });
    }
    res.json({ success: true, data: user });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});
